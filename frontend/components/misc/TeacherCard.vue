<template>
  <NuxtLink
    :to="
      `/teacher/${teacher.name.toLowerCase()}-${teacher.surname.toLowerCase()}`.replace(
        /\s+/g,
        '-',
      )
    "
  ...
  >
    ...
    <div class="absolute bottom-0 left-0 right-0 p-6">
      <div class="flex flex-wrap gap-2 items-center mb-2">
        <span
          class="bg-primary bg-opacity-80 text-white px-3 py-1 rounded-full text-sm group-hover:bg-opacity-100 transition-all duration-300 group-hover:translate-y-[-2px]"
        >
          {{ teacher.role }}
        </span>
      </div>
      <h1
        class="text-2xl font-bold text-white group-hover:translate-y-[-2px] transition-all duration-300"
      >
        {{ teacher.name }} {{ teacher.surname }}
      </h1>
      <div class="flex flex-wrap gap-2 mt-2">
        <span
          v-if="teacher.main_expertise && teacher.main_expertise.includes('&')"
