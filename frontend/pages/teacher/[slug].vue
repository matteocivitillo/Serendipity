<template>
  <div class="flex flex-col min-h-screen bg-gray-100 dark:bg-gray-800">
    ...
    <main class="flex-grow">
      ...
      <div
        v-if="isLoading"
        class="flex justify-center items-center h-96 bg-white dark:bg-gray-700"
      >
        <div class="loading-spinner animate-fade-in">
          <div class="spinner"></div>
          <p
            class="mt-4 text-xl text-gray-600 dark:text-gray-300 animate-fade-in"
          >
            {{ t("loadingTeacher") }}
          </p>
        </div>
      ...
      <template v-else-if="teacher">
          ...
          <div class="container mx-auto px-6 md:px-10 lg:px-20">
            ...
            <div class="flex flex-col md:flex-row gap-8 justify-center">
              ...
              <div class="w-full md:w-2/5 lg:w-1/3 teacher-card">
                <div
                  class="rounded-xl shadow-md overflow-hidden bg-white dark:bg-gray-700"
                >
                  <div class="h-96 relative">
                    <img
                      :src="
                        teacher.image ||
                        `/images/teachers/${teacher.name.toLowerCase()}-${teacher.surname.toLowerCase()}.jpg`
                      "
                    ...
                    <div class="absolute inset-x-0 bottom-0 p-6">
                      <div class="flex flex-wrap gap-2 items-center mb-2">
                        <span
                          class="bg-primary bg-opacity-80 text-white px-3 py-1 rounded-full text-sm animate-fade-in"
                        >
                          {{ teacher.role }}
                        </span>
                      </div>
                      ...
                      <div class="flex flex-wrap gap-2 mt-2">
                        <span
                          v-if="
                            ...
                            teacher.main_expertise.includes('&')
                          "
              ...
              <div class="w-full md:w-3/5 lg:w-1/2 teacher-card">
                <div
                  class="bg-white dark:bg-gray-700 rounded-xl shadow-md p-8 h-full"
                >
                  <!-- Teacher Name and Title -->
                  <div
                    class="mb-6 border-b border-gray-300 dark:border-gray-600 pb-4"
                  >
                    <h1
                      class="text-4xl font-bold text-primary dark:text-[#9ACBD0] animate-fade-in"
                    >
                      {{ teacher.name }} {{ teacher.surname }}
                    </h1>
                  </div>
                  ...
                  <div class="flex flex-col sm:flex-row gap-8">
                    ...
                    <div class="w-full sm:w-1/2 space-y-6">
                      ...
                      <div class="space-y-2">
                        <span
                          class="bg-primary bg-opacity-80 text-white px-4 py-1.5 rounded-full text-base inline-block mb-2 animate-fade-in"
                        >
                          {{ t("role") }}
                        </span>
                        <p
                          class="text-xl text-gray-600 dark:text-gray-300 animate-fade-in"
                        >
                          {{ teacher.role }}
                        </p>
                      </div>
                      ...
                      <div class="space-y-2">
                        <span
                          class="bg-[#48A6A7] text-white px-4 py-1.5 rounded-full text-base inline-block mb-2 animate-fade-in"
                        >
                          {{ t("expertise") }}
                        </span>
                        <p
                    ...
                    <div class="w-full sm:w-1/2 space-y-2">
                      <span
                        class="bg-white dark:bg-gray-700 bg-opacity-80 border border-primary dark:border-[#9ACBD0] text-primary dark:text-[#9ACBD0] px-4 py-1.5 rounded-full text-base inline-block mb-2 animate-fade-in"
                      >
                        {{ t("about") }}
                      </span>
                      <p
                        class="text-lg text-gray-600 dark:text-gray-300 animate-fade-in"
                      >
                        {{ teacher.about }}
                      </p>
                    </div>
        ...
        <section
          class="py-14 bg-white dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600"
        >
          <div
            class="container mx-auto px-6 md:px-20 flex flex-col items-center"
          >
            <h2
              class="text-3xl font-bold text-primary dark:text-[#9ACBD0] mb-8 animate-fade-in text-center"
            >
              {{ t("activities") }}
            </h2>
            ...
            <div
              ...
              class="text-gray-600 dark:text-gray-300 text-lg animate-fade-in p-8 bg-gray-50 dark:bg-gray-600 rounded-lg text-center"
            >
              {{ t("noActivities") }}
            </div>
            ...
            <div
              v-else
              class="flex flex-wrap justify-center gap-6 w-full max-w-5xl"
            >
              <div
                v-for="(activity, index) in activities"
                :key="activity.id"
                class="w-full sm:w-[calc(50%-12px)] md:w-[calc(40%-16px)] flex justify-center"
              >
                <ActivityCard
                  :activity="activity"
...
const translations = {
  en: {
    ...
    seoDescription:
      "Meet our dedicated teachers and learn more about their expertise and teaching style.",
  ...
  it: {
    ...
    seoTitle: "{name} {surname} - Serendipity Yoga",
    seoDescription:
      "Incontra i nostri dedicati insegnanti e scopri di più sulla loro specialità e il loro stile di insegnamento.",
  ...
  fr: {
    ...
    seoTitle: "{name} {surname} - Serendipity Yoga",
    seoDescription:
      "Rencontrez nos professeurs dévoués et découvrez leur expertise et leur style d'enseignement.",
  ...
  de: {
    ...
    seoTitle: "{name} {surname} - Serendipity Yoga",
    seoDescription:
      "Lernen Sie unsere engagierten Lehrer kennen und erfahren Sie mehr über ihre Expertise und ihren Unterrichtsstil.",
...
const findTeacherIdBySlug = async () => {
  try {
    ...
    const response = await fetch(
      `${API_URL}/teachers?lang=${selectedLang.value}`,
    );
    ...
    if (data.teachers && data.teachers.length > 0) {
      data.teachers.forEach((t) => {
        const teacherSlug =
          `${t.name.toLowerCase()}-${t.surname.toLowerCase()}`.replace(
            /\s+/g,
            "-",
          );
      });
      ...
      for (const teacher of data.teachers) {
        ...
        if (
          teacherNameLower.includes(nameInSlug) ||
      ...
      for (const teacher of data.teachers) {
        for (const part of slug.value.split("-")) {
          if (part.length > 3) {
            ...
            const teacherFullNameLower =
              `${teacher.name} ${teacher.surname}`.toLowerCase();
            if (
              teacherFullNameLower.includes(part) ||
              part.includes(teacher.name.toLowerCase())
            ) {
              teacherId.value = teacher.id;
              return teacher.id;
            }
...
const fetchTeacher = async () => {
  ...
  try {
    const response = await fetch(
      `${API_URL}/teacher/${teacherId.value}?lang=${selectedLang.value}`,
    );
...
const fetchActivities = async () => {
  ...
  try {
    const response = await fetch(
      `${API_URL}/teacher/${teacherId.value}/activities?lang=${selectedLang.value}`,
    );
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
...
.spinner {
  ...
  animation:
    spin 1.2s cubic-bezier(0.5, 0.1, 0.5, 1) infinite,
    scale 0.8s cubic-bezier(0.25, 0.1, 0.25, 1) forwards; /* Slower animations */
}
